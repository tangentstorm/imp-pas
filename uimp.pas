...
type
  TKind = (
    ...
    kMF0,  // a meta-function (() -> TExpr, implemented in pascal)
    ...
    kMF5); // a meta-function (a,b,c,d:TExpr -> TExpr)
...
// functions of up to 5 arguments.
//
// In order to make these a little more flexible, I came back
// and added 'of object'. This means they have to be methods.
// We could have had separate types to distinguish plain
// functions from methods, but it seemed like methods would
// be the more useful case.
type
  TMetaFun0 = function : TExpr of object;
  TMetaFun1 = function (a : TExpr) : TExpr of object;
  TMetaFun2 = function (a,b : TExpr) : TExpr of object;
  TMetaFun3 = function (a,b,c : TExpr) : TExpr of object;
  TMetaFun4 = function (a,b,c,d : TExpr) : TExpr of object;
  TMetaFun5 = function (a,b,c,d,e : TExpr) : TExpr of object;

...
const
  aritykind : array[TArity] of TKind = (kMF0, kMF1, kMF2, kMF3, kMF4, kMF5);

var
  metas     : TMetaTbl;
  metacount : byte = 0;

// NextMeta adds a function record to the 'metas' table and constructs
...
function L : TExpr; inline;
function L( a : TExpr ) : TExpr; inline;
...
var
  ...
  sAssoc, sSublis,  sApply, sEval, sAppq,
...
// nx : numeric expression
function Nx( n : integer ) : TExpr;

//-- f. universal evaluator ------------------------------------
// ( already covered )
//  TODO : maybe break this down into separate interfaces?
...
function NextMeta( a : TArity; var p ) : TExpr;
  begin
    if metacount = high(metas) then
      die( 'out of meta slots.' )
    else begin
...
function TMetaLang.mCAR( x : TExpr ) : TExpr;
  begin
...
function TMetaLang.mCDR( x : TExpr ) : TExpr;
  begin
...
// 5. cons[x;y] -> (x, y)
function TMetaLang.mCONS( x, y : TExpr ) : TExpr;
  var cell : TCell;
  begin
    cell.car := x;
    cell.cdr := y;
...
function TMetaLang.mATOMP( x : TExpr ) : TExpr;
  begin
    result := EnBool( mATOM( x ))
  end;

function TMetaLang.mEQP( x, y : TExpr ) : TExpr;
  begin
...
function TMetaLang.mSUBST( x, y, z : TExpr ) : TExpr;
  begin
    if mATOM(z) then
...
function TMetaLang.mEQUAL( x, y : TExpr ) : TExpr;
  begin
    result := EnBool(
...
function TMetaLang.mNULLP( x : TExpr ) : TExpr;
  begin
    result := mEQP(x, sNULL)
  end;

...
// caar[x] -> car[car[x]]
function TMetaLang.mCAAR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCAR(x))
  end;

// cadr[x] -> car[cdr[x]]
function TMetaLang.mCADR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCDR(x))
  end;

function TMetaLang.mCDDR( x : TExpr ) : TExpr;
  begin
    result := mCDR(mCDR(x))
  end;

// cadar[x] -> car[cdr[cdr[x]]]
function TMetaLang.mCADAR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCDR(mCAR(x)))
  end;

// caddar[x] -> car[cdr[cdr[car[x]]]]
function TMetaLang.mCADDAR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCDR(mCDR(mCAR(x))))
  end;

// caddr[x] -> car[cdr[cdr[x]]]
function TMetaLang.mCADDR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCDR(mCDR(x)))
  end;

function TMetaLang.mCADDDR( x : TExpr ) : TExpr;
  begin
    result := mCAR(mCDR(mCDR(mCDR(x))))
  end;

function TMetaLang.mCADDDDR( x : TExpr ) : TExpr;
  begin
...
function L( a : TExpr ) : TExpr;
  begin result := ml.mCONS(a, sNULL) end;
...
// (list a (b c)) -> (a (b c))
function L( a, b : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b)) end;

// The rest of these just follow the same pattern:

function L( a, b, c : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c)) end;

function L( a, b, c, d : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d)) end;

function L( a, b, c, d, e : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e)) end;

function L( a, b, c, d, e, f : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e, f)) end;

function L( a, b, c, d, e, f, g : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e, f, g)) end;

function L( a, b, c, d, e, f, g, h : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e, f, g, h)) end;

function L( a, b, c, d, e, f, g, h, i : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e, f, g, h, i)) end;

function L( a, b, c, d, e, f, g, h, i, j : TExpr ) : TExpr; inline;
  begin result := ml.mCONS(a, L(b, c, d, e, f, g, h, i, j)) end;

// - list enumerator - - - - - - - - - - - - - - - - - - - - - -
...
function TExprEnumerator.MoveNext: boolean;
  begin
    ...
    else self.cell := ml.mCDR(self.cell);
...
function TMetaLang.mAPPEND( x, y : TExpr ) : TExpr;
  begin
...
function mAMONG( x, y : TExpr ) : boolean;
  begin
    result := mEQ(x,ml.mCAR(y)) or mAMONG(x, ml.mCDR(y))
  end;

function TMetaLang.mAMONGP( x, y : TExpr ) : TExpr;
  begin
    result := EnBool(mAMONG(x, y))
  end;

// zip -- McCarthy calls this 'pair'. 'zip' comes from haskell and python.
function TMetaLang.mZIP( x, y : TExpr ) : TExpr;
  begin
...
function TMetaLang.mASSOC( s, a : TExpr ) : TExpr;
  begin
...
function TMetaLang.mSUBLIS( x, y : TExpr ) : TExpr;
...
procedure TMetaLang.CreateBuiltins;
  begin
...
function VL(vars : array of variant) : TExpr;
  ...
  begin
    ...
    for i := high(vars) downto 0 do
      result := ml.mCONS(Vx(vars[i]), result)
  end; { VL }
...
function TMetaLang.mAPPQ( m : TExpr ) : TExpr;
  begin
    if mNULL(m) then result := sNULL
    else result := mCONS(q(mCAR(m)), mAPPQ(mCDR(m)))
  end;

function TMetaLang.mAPPLY( f, args, env : TExpr ) : TExpr;
  begin
    result := mEVAL(mCONS(f, mAPPQ(args)), env)
  end;

function TMetaLang.mEVAL( e, a : TExpr ) : TExpr;
...

function TMetaLang.mBIND( iden, value : TExpr ) : TExpr;
  begin
    mENV := mCONS(L(iden, value), mENV); // flat list of pairs for now
    result := iden;
  end;

function TMetaLang.mLIST ( x : TExpr ) : TExpr;
  begin
    raise ENotImplementedError.Create('mLIST');
  end;

//-- g. higher order functions ---------------------------------

function TMetaLang.mMAPLIST( f, x : TExpr ) : TExpr;
  begin
    raise ENotImplementedError.Create('mMAPLIST');
  end;

function TMetaLang.mSEARCH( f, x : TExpr ) : TExpr;
  begin
    raise ENotImplementedError.Create('mSEARCH');
  end;

function TMetaLang.mFILTER( f, x : TExpr ) : TExpr;
  begin
    raise ENotImplementedError.Create('mFILTER');
  end;

function TMetaLang.mREDUCE( f, x, y : TExpr ) : TExpr;
  begin
    raise ENotImplementedError.Create('mREDUCE');
  end;

//-- arithmetic  -----------------------------------------------
...

function TMetaLang.mADD( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data + y.data)
  end;

function TMetaLang.mSUB( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data - y.data)
  end;

function TMetaLang.mMUL( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data * y.data)
  end;

function TMetaLang.mDIV( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data div y.data)
  end;

function TMetaLang.mMOD( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data mod y.data)
  end;

 function TMetaLang.mPOW( x, y : TExpr ) : TExpr;
  begin
    if ints(x,y,result) then result := Nx(x.data ** y.data)
  end;

...

function TMetaLang.mREADFILE  ( path : TExpr ) : TExpr;
  begin
...
function TMetaLang.mWRITEFILE ( path, data : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mWRITEFILE }

function TMetaLang.mBINREAD   ( path : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mBINREAD }

function TMetaLang.mBINWRITE  ( path, data : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mBINWRITE }

function TMetaLang.mARRAY ( size : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mARRAY }

function TMetaLang.mLEN ( a : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mLEN }

function TMetaLang.mPUSH ( a, x : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mPUSH }

function TMetaLang.mPOP ( a, x : TExpr ) : TExpr;
  begin
    result := sNULL
  end; { mPOP }

...
function TMetaLang.mSYM2CHARS ( x : TExpr ) : TExpr;
  ...
  begin
    if x.kind = kERR then result := x
	  ...
	  result := ml.mCONS(Sym(s[i]), result)
      end
